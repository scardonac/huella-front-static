...
export const AirConditionersReportU = () => {
    ...
    const { control, handleSubmit, reset, clearErrors, setValue, setError, getValues, formState: { errors } } = useForm({
        defaultValues: {
            airConditioners: [
                { nameForm: 'Aires acondicionados', flagNameForm: false, gasType: '', pounds: '', numberOfAirs: '', attachedFiles: [null] },
            ]
        }
    });
    ...
    const handleFileChange = (e, formIndex, fileIndex) => {
        ...
        if (file && allowedExtensions.includes(`.${fileExtension}`)) {
            ...
            const updatedAirConditioners = [...values.airConditioners];
            updatedAirConditioners[formIndex].attachedFiles[fileIndex] = file;
            setValue('airConditioners', updatedAirConditioners);
            clearErrors('airConditioners.attachedFiles');
        } else {
            ...
            setError('airConditioners.attachedFiles', {
                type: 'manual',
    ...
    const addFile = (formIndex) => {
        const values = getValues();
        const updatedAirConditioners = [...values.airConditioners];
        updatedAirConditioners[formIndex].attachedFiles.push(null);
        setValue('airConditioners', updatedAirConditioners);
    };
    const removeFile = (formIndex, fileIndex) => {
        const values = getValues();
        const updatedAirConditioners = [...values.airConditioners];
        updatedAirConditioners[formIndex].attachedFiles.splice(fileIndex, 1);
        setValue('airConditioners', updatedAirConditioners);
    ...
    const toggleFlagNameForm = (formIndex) => {
        const values = getValues();
        const updatedAirConditioners = [...values.airConditioners];
        updatedAirConditioners[formIndex].flagNameForm = !updatedAirConditioners[formIndex].flagNameForm;
        setValue('airConditioners', updatedAirConditioners);
    };
    ...
    const handleUpdateNameForm = (formIndex, value) => {
        const values = getValues();
        const updatedAirConditioners = [...values.airConditioners];
        updatedAirConditioners[formIndex].nameForm = value;
        setValue('airConditioners', updatedAirConditioners);
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col items-center justify-center'>
                {[...fields].sort().map((_, formIndex) => (
                    <div className='flex flex-col items-center justify-center gap-4 pt-6 w-full' key={formIndex}>
                        ...
                        <Controller
                            control={control}
                            name={`airConditioners[${formIndex}].nameForm`}
                            render={({ field }) =>
                        ...
                        <Controller
                            control={control}
                            name={`airConditioners[${formIndex}].gasType`}
                            ...
                            render={({ field }) =>
                                <div className='flex flex-col w-2/4'>
                                    ...
                                    <select {...field} className='mt-1 block w-full pl-3 pr-10 py-2 text-base border-[0.5px] border-[#627173] bg-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md'>
                                        ...
                                        <option value="1">Gas 1</option>
                                        <option value="2">Gas 2</option>
                                        <option value="3">Gas 3</option>
                                        <option value="4">Gas 4</option>
                                    </select>
                                    {errors.airConditioners && errors.airConditioners[formIndex]?.gasType && (
                                        <CustomAlert
                                            message={errors.airConditioners[formIndex]?.gasType.message}
                                            type='error'
                                        />
                                    )}
                                </div>
                        ...
                        <Controller
                            control={control}
                            name={`airConditioners[${formIndex}].numberOfAirs`}
                            ...
                            render={({ field }) =>
                                <div className='flex flex-col w-2/4'>
                                    ...
                                    <input
                                        ...
                                        type='number'
                                    />
                                    {errors.airConditioners && errors.airConditioners[formIndex]?.numberOfAirs && (
                                        <CustomAlert
                                            message={errors.airConditioners[formIndex]?.numberOfAirs.message}
                                            type='error'
                                        />
                                    )}
                                </div>
                        ...
                        {errors?.airConditioners && errors?.airConditioners?.attachedFiles && (
                            <CustomAlert
                                message={errors?.airConditioners?.attachedFiles?.message}
                                type='error'
                            />
                        )}
                        ...
                        <div className={`flex items-center w-2/4 text-darkgray ${formIndex !== fields.length - 1 ? 'hidden' : null}`}>
                            ...
                            <div className='bg-primary-green2 bg-no-repeat px-4 py-2 rounded-lg opacity-100 cursor-pointer' onClick={() =>
                                append({ nameForm: 'Aires acondicionados', flagNameForm: false, gasType: '', pounds: '', numberOfAirs: '', attachedFiles: [null] },)}>
                                <b className='tracking-tighter leading-6 text-primary-80 font-bold text-left text-base text-primary-title1 opacity-100'>
                                    Agregar otro tipo de aire acondicionado
                                </b>
                            </div>
                        </div>
                    </div>
